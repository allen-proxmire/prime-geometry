#!/usr/bin/env python3
"""
PG03_02_blocksize_dependence_curvature_purepy.py

Mirror of PG03_01, but applied to the curvature invariant chi_n.
Studies how curvature ordering coherence decays with block size.
...
def block_permute(x, block, rng):
    n = len(x)
    m = n // block
    blocks = x[:m * block].reshape(m, block)
    out = blocks[rng.permutation(m)].reshape(-1)
    if m * block < n:
        out = np.concatenate([out, x[m * block :]])
    return out
...
def main():
    ...
    print(f"Number of curvature points: {len(chi)}")

    rho_prime = spearman_curve(chi, args.max_lag)
    rng = np.random.default_rng(args.seed)

    print("\n=== CURVATURE BLOCK SIZE DEPENDENCE ===")
    ...
    for B in block_sizes:
        ...
        for t in range(args.trials):
            chi_perm = block_permute(chi, B, rng)
            null_curves[t] = spearman_curve(chi_perm, args.max_lag)
    ...
    out = "PG03_02_blocksize_dependence_curvature.csv"
