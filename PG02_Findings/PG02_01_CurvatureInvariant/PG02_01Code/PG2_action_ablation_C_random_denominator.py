...
# ============================================================
# ABLATION C: Randomized denominator
# χ_n = (g_{n+2} - g_n) / permuted(g_n + g_{n+1})
# ============================================================
def action_random_denominator(gaps: np.ndarray, denom_perm: np.ndarray) -> float:
    g0 = gaps[:-2].astype(np.float64)
    g1 = gaps[1:-1].astype(np.float64)
    g2 = gaps[2:].astype(np.float64)

    num = (g2 - g0)
    den = denom_perm.astype(np.float64)
...
# IMPORTANT: denom_perm is held fixed for this block size
# ============================================================
def run_block_test(gaps, block_size, trials, rng):
    # Correct denominators for the given gap sequence:
    # d_n = g_n + g_{n+1} for n=0..len(gaps)-2, but chi uses n up to len(gaps)-3
    denom = (gaps[:-2] + gaps[1:-1]).astype(np.int64)  # length = len(gaps)-2
    # Randomly permute denominators ONCE (fixed scramble)
    denom_perm = rng.permutation(denom)

    S_prime = action_random_denominator(gaps, denom_perm)
    ...
    for i in range(trials):
        ...
        # IMPORTANT: For fairness, we compute each action using a denominator
        # derived from that sequence, then permuted (same “destroy geometry” operation).
        denom_p = rng.permutation((gp[:-2] + gp[1:-1]).astype(np.int64))
        denom_b = rng.permutation((gb[:-2] + gb[1:-1]).astype(np.int64))

        S_perm[i]  = action_random_denominator(gp, denom_p)
        S_block[i] = action_random_denominator(gb, denom_b)
...
print("\nABLATION C — RANDOMIZED DENOMINATOR (GEOMETRY DESTROYED)")
print("-------------------------------------------------------------")
