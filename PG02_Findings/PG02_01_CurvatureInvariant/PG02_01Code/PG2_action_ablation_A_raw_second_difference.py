...
# ============================================================
# ABLATION A: Raw second difference curvature
# χ_n = g_{n+2} - 2 g_{n+1} + g_n
# ============================================================
def curvature_action_raw_second_difference(gaps: np.ndarray) -> float:
...
def run_block_test(gaps, block_size, trials, rng):
    S_prime = curvature_action_raw_second_difference(gaps)

    S_perm = np.empty(trials)
    S_block = np.empty(trials)

    for i in range(trials):
        S_perm[i]  = curvature_action_raw_second_difference(permute_gaps(gaps, rng))
        S_block[i] = curvature_action_raw_second_difference(block_permute_gaps(gaps, block_size, rng))
...
print("\nABLATION A — RAW SECOND DIFFERENCE")
print("-------------------------------------------------------------")
