...
# ============================================================
# ABLATION B: Wrong denominator curvature
# χ_n = (g_{n+2} - g_n) / g_{n+1}
# ============================================================
def curvature_action_wrong_denominator(gaps: np.ndarray) -> float:
...
def run_block_test(gaps, block_size, trials, rng):
    S_prime = curvature_action_wrong_denominator(gaps)

    S_perm = np.empty(trials)
    S_block = np.empty(trials)

    for i in range(trials):
        S_perm[i]  = curvature_action_wrong_denominator(permute_gaps(gaps, rng))
        S_block[i] = curvature_action_wrong_denominator(
            block_permute_gaps(gaps, block_size, rng)
        )
